...
const l10n = new Localization([
  ...
  "chrome://zoteroduplicatesmerger/locale/overlay.ftl",
]);
...
async function loadIntoWindow(window) {
  ...
  let mergeLabel = await getLocalizedString(
    "zotero.zoteroduplicatesmerger.menu.label"
  );
  let toolsMenuLabel = await getLocalizedString(
    "zotero.zoteroduplicatesmerger.toolsmenu.label"
  );

  let menuItem = document.createElement("menuitem");
  menuItem.setAttribute("label", mergeLabel);
  menuItem.setAttribute("id", "zoteroduplicatesmerger-menu");
